...
Das Projekt ist bewusst simpel gehalten, um Wartbarkeit und Anpassungen zu erleichtern:

    ev-manager/
    ├── app.py              # Hauptanwendung (Flask Server & Logik)
    ├── data.csv            # Datenspeicher (Charge Logs)
    ├── settings.json       # Konfiguration (Standardwerte)
    ├── static/
